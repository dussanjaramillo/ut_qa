...
<table id="tabla1">
    ...
    <tbody>
        ...
        if ($consulta) {
            $n = 0;
            /* PROCESOS DE RECEPCION TITULOS */
            foreach ($consulta['titulos'] as $data2) {
                ?> 
                <tr><td><?php ?></td>
                    <td><?php echo $data2['PROCESOPJ']; ?></td>
                    <td><?php echo $data2['FECHA_CONSULTAONBASE']; ?></td>
                    <td>
                        <a target="_blank"  class="btn btn-small btn-info" title="ver" onclick="ver_detalle_proceso('<?php echo "detalle_deuda" . $n ?>')">
                            <i class="fa fa-eye "></i>
                        </a>
                        <div id="<?php echo "detalle_deuda" . $n ?>" style="display:none">
                            <table width="431" border="1" align="center" class="table table-bordered table-striped">
                                <tr><td colspan="7"><center> Expediente N° <?php echo $data2['PROCESOPJ']; ?></center></td></tr>
                                <tr>
                                ...
                                <td width="34"><div align="center"><?php echo $data2['PROCESOPJ']; ?></div></td>
                                <td width="60"><div align="center"> <?php echo $data2['CONCEPTO']; ?> </div></td>
                                <td width="111"><div align="center"><input type="text" readonly="readonly" name="<?php echo "p_saldo_deuda_" . $n; ?>" id="<?php echo "p_saldo_deuda_" . $n ?>" class="input-small uneditable-input" value="<?php echo number_format($data2['SALDO_DEUDA']) ?>"></div></td>
                                <td width="89"><div align="center"><input type="text" readonly="readonly" name="<?php echo "p_saldo_capital_" . $n; ?>" id="<?php echo "p_saldo_capital_" . $n ?>" class="input-small uneditable-input" value="<?php echo number_format($data2['SALDO_CAPITAL']) ?>"></div></td>
                                <td width="103"><div align="center"><input type="text" readonly="readonly" name="<?php echo "p_saldo_interes_" . $n; ?>" id="<?php echo "p_saldo_interes_" . $n ?>" class="input-small uneditable-input" value="<?php echo number_format($data2['SALDO_INTERES']) ?>"></div></td>
                                <td width="103"><div align="center"><input type="text" readonly="readonly" name="<?php echo "p_saldo_sancion_" . $n; ?>" id="<?php echo "p_saldo_sancion_" . $n ?>" class="input-small uneditable-input" value="<?php echo "0" ?>"></div></td>
                                <td width="103"><div align="center"><input type="text" readonly="readonly" name="<?php echo "p_saldo_multa_" . $n; ?>" id="<?php echo "p_saldo_multa_" . $n ?>" class="input-small uneditable-input" value="<?php echo "0" ?>"></div></td>
                            </table>  
                        </div> 
                    </td>
                    ...
                    <td>
                        ...
                        switch ($cargo):
                            case 7:
                                echo NOMBRE_SECRETARIO . '<br>';
                                break;
                            case 8:
                                echo strtoupper($data2['NOMBRES'] . " " . $data2['APELLIDOS']) . '<br>';
                                break;
                            case 9:
                                echo NOMBRE_COORDINADOR . '<br>';
                                break;
                        endswitch;
                        ?>
                    </td>
                    <td>     
                        <?php
                        $cantidad = count($data2['ABOGADO']);
                        if ($cantidad > 0):
                            $cod_abogado = $data2['ABOGADO'];
                        else:
                            $cod_abogado = 0;
                        endif;
                    ...
                    <td> <a target="_blank" class="btn btn-small btn-info" title="ver" onclick=" f_enviar_titulos('<?php echo $data2['CONCEPTO'] ?>', '<?php echo $data2['PROCESOPJ'] ?>', '<?php echo $data2['IDENTIFICACION'] ?>', '<?php echo $data2['COD_RESPUESTA'] ?>', '<?php echo $n; ?>', '<?php echo $data2['COD_REGIONAL'] ?> ', '<?php echo $cod_abogado ?>', '<?php echo $data2['PROCESO'] ?>');">
                            <i class="fa fa-eye "></i>
                        </a>
                        <div id="<?php echo "responsabletitulos_" . $n; ?>">
                        </div></td>
                    <td> <form name="form1" id="form1" method="post" target="_blank" action="<?php echo $ruta_traza ?>">
                            <input type="hidden" id="FISCALIZACION" name="FISCALIZACION" value="<?php echo $data2['PROCESO']; ?>" />
                            <input type="submit" class="btn btn-info" name="Abrir" id="Abrir"  value="Abrir">
                        </form></td>
                    <td>
                    </td>
                    <td>
                        <?php if (ID_USUARIO == $data2['ABOGADO']): ?>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php
            ...
            /* FIN PROCESOS DE RECEPCION TITULOS */

            /* PROCESOS COACTIVOS   */
            $m = $n;
            foreach ($consulta['procesos'] as $data) {
                ?> 
                <tr>
                    <td></td>
                    <td>
                        ...
                        <?php
                        $respuestas = explode("*?*", $data['CODIGOS RESPUESTAS']);
                        $ver_proceso = FALSE;
                        ...
                        ?></td>
                    <td><?php
                        $fechas = explode("?*", $data['FECHAS RECEPCION']);
                        echo $fechas[0];
                        ?>
                    </td>
                        ...
                        <div id="<?php echo "detalle_deuda" . $m ?>" style="display:none">
                            ...
                            <table width="431" border="1" align="center" class="table table-bordered table-striped">
                                <tr>
                                    <td colspan="7"><?php
//                                
                                        $expedientes = explode("?*", $data['NUMEROS EXPEDIENTES']);
                                        $conceptos_deuda = explode("?*", $data['CONCEPTOS UNIDOS']);
                                        $saldo_deuda = explode("?*", $data['SALDOS DEUDAS']);
                                        $saldo_capital = explode("?*", $data['SALDOS CAPITAL']);
                                        $saldo_interes = explode("?*", $data['SALDOS INTERESES']);
                                ...
                                for ($v = 0; $v < count($expedientes); $v++):
                                    if (!empty($saldo_deuda[$v])):
                                        ...
                                        <tr>
                                            ...
                                            <td width="111"><div align="center"><input type="text" readonly="readonly" name="<?php echo "p_saldo_deuda_" . $m . "_" . $v; ?>" id="<?php echo "p_saldo_deuda_" . $m . "_" . $v ?>" class="input-small uneditable-input info" value="<?php echo number_format($saldo_deuda[$v]) ?>"></div></td>
                                            <td width="89"><div align="center"><input type="text" readonly="readonly" name="<?php echo "p_saldo_capital_" . $m . "_" . $v; ?>" id="<?php echo "p_saldo_capital_" . $m . "_" . $v ?>" class="input-small uneditable-inpu info" value="<?php echo number_format($saldo_capital[$v]) ?>"></div></td>
                                            <td width="103"><div align="center"><input type="text" readonly="readonly" name="<?php echo "p_saldo_interes_" . $m . "_" . $v; ?>" id="<?php echo "p_saldo_interes_" . $m . "_" . $v ?>" class="input-small uneditable-inpu" value="<?php echo number_format($saldo_interes[$v]) ?>"></div></td>
                                ...
                                <?php if ($sum_deuda): ?>
                                    <tr>
                                        <td width="34" colspan="4"><div align="center">Totales</div></td>
...
//                        echo $data['RESPUESTAS UNIDAS'];
                        ...
                        $estado = explode("*?*", $data['CODIGOS RESPUESTAS']);
...
<div id="procesos_transversales" style="text-align:center;display:block"  >
    ...
    <br>
    <br>
    <button class="btn btn-success"  style="width:210px;display: none  " name="boton_facilidad" id="boton_facilidad" type="button" value="" onclick="enviar_proceso(this.value, '<?php echo $ruta_facilidad; ?>');">Facilidad de Pago</button>
    <br>
</div>
...
//        if(url_formulario=='<?php echo $ruta_facilidad ?>'){
//            alert('adsdsd');
//          //  $("#form_proceso").target='_new';
//        }
        $("#form_proceso").submit();
    }
    ...
    function f_procesos(cod_coactivo, visualiza) {
        ...
        $("#boton_facilidad").val(cod_coactivo);
