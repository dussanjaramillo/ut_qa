...
<h1>Consulta Entidades</h1>
<br>


<form action="<?php echo base_url('index.php/consultarprocesos/actualizatraza'); ?>" name="form6" id="form6" method="POST">
    <table cellspacing="20" align="center">
        ...
        <tr>
            <td style="margin: 10px; padding: 10px;">
                <label>Numero Proceso Coactivo: </label>
            </td>

            <td style="margin: 10px; padding: 10px;">
                <input name="COACTIVO" id="COACTIVO" onkeypress="return  soloLetras(event)">
            </td>
            
            <td>
                <label>Origen Proceso: </label>
            </td>
            <td>
                <select name="ORIGEN" id="ORIGEN">
                    <option value="">Seleccione Algun Origen</option>
                    <option value="AD">Proceso Administrativo</option>
                    <option value="JR">Proceso Juridico</option>
                    <option value="PJ">Proceso Judicial</option>
                    <option value="DV">Proceso Devoluciones</option>
                    <option value="NM">Cartera No Misional</option>
                </select>
            </td>
        ...
        <tr>
            ...
            <td></td>
            <td></td>
            <td align="right" style="margin: 10px; padding: 10px;text-align:right">
                <br>
...
<table id="traza">
    <thead>
        <tr>
            ...
            <th style="display:none">N&uacutemero Titulo</th>
            ...
            <th>N&uacute;mero de Proceso en Coactivo</th>
            <th>CIIU</th>
    ...
    <tbody>
        ...
        foreach ($datos->result_array as $col) {
            ...
            <tr>
                <?php
                if($col['VISTA_ORIGEN']=='AD'){
                    echo '<td>Proceso Administrativo</td>';
                }else if($col['VISTA_ORIGEN']=='NM'){
                    echo '<td>Cartera No Misional</td>';
                    
                }else if($col['VISTA_ORIGEN']=='PJ'){
                    echo '<td>Proceso Judicial</td>';
                    
                }else if($col['VISTA_ORIGEN']=='DV'){
                    echo '<td>Devoluciones</td>';
                    
                }else if($col['VISTA_ORIGEN']=='JR'){
                    echo '<td>Proceso Juridico</td>';
                    
                }else{
                    echo '<td></td>';
                }
                ?>
                <!--<td><?= $col['VISTA_ORIGEN'] ?></td>-->
                
                <td><?= $col['COD_PROCESO_COACTIVO'] ?></td>
                <td style="display:none"><?= $col['TITULO'] ?></td>
                <td><?= $col['NIT'] ?></td>
                <td><?= $col['NOMEMPRESA'] ?></td>
                <td><?= $col['TELEFONO'] ?></td>
                <td><?= $col['DIRECCION'] ?></td>
                <td style="display:none"><?= $col['DEPARTAMENTO'] ?></td>
                <td> <?= $col['NOMDEPARTAMENTO'] ?></td>
                <td style="display:none"><?= $col['REGIONAL'] ?></td>
                <td><?= $col['NOMREGIONAL'] ?></td>
                <td style="display:none"><?= $col['REGIMEN'] ?></td>
                <td><?= $col['CODIGOPJ'] ?></td>
                <td><?= $col['CIIU'] ?></td>
                <td><?= $col['FECHA_CONSULTAONBASE'] ?></td>
                <td><?= $col['FECHA_EJECUTORIA'] ?></td>
                <td><?= $col['FECHA_RESOLUCION'] ?></td>
                <td>
                    <form action= "<?php
                    $tal = $col['COD_PROCESO_COACTIVO'];
                ...
                <td>
                    <button class="btn btn-info at" id="traza" name="traza" at="<?= $col['COD_PROCESO_COACTIVO'] ?>" onclick="gestion('<?= $col['COD_PROCESO_COACTIVO'] ?>','<?= $col['TITULO'] ?>','<?= $col['VISTA_ORIGEN'] ?>')"><i class="fa fa-eye"> Ver</i></button>

                </td>
                <td align="center">
                    <form id="form" name="form" target = "_blank"  method="post" action="<?php echo base_url('index.php/consultarprocesos/pdfproceso') ?>">
                        <br>
                        <button type="submit" name="pdf" id="pdf" class="btn btn-info"><i class="fa fa-file-pdf-o"> Pdf</i></button>
                        <input type="hidden" name="cod_fiscalizacion" id="cod_fiscalizacion" value="<?php echo $col['COD_PROCESO_COACTIVO']; ?>" >
                        <input type="hidden" name="cod_titulo" id="cod_titulo" value="<?php echo $col['TITULO']; ?>" >
                        <input type="hidden" name="vista" id="vista" value="<?php echo $col['VISTA_ORIGEN']; ?>" >
                    </form>
                </td>
...
//    $(".at").click(function() {
        ...
        var url = "<?= base_url('index.php/consultarprocesos/verproceso') ?>";

        $('#cargar').load(url, {id: id,titulo:titulo,vista:vista});
...
//    });
    ...
    function valida() {
        if ($("#CODEMPRESA").val() == "" && $("#NOMEMPRESA").val() == "" && $("#CIUDAD").val() == "" && $("#REGIONAL").val() == "" && $("#FISCALIZACION").val() == "") {
            document.forms.form6.action = "<?php echo base_url("index.php/consultarprocesos/consultatraza") ?>";
        }
