...
<h1>Consulta Traza Por Entidades Proceso  Jur&iacute;dico</h1>
<br>


<form action="<?php echo base_url('index.php/consultarprocesos/actualizatrazajuridico'); ?>" name="form6" id="form6" method="POST">
    <table cellspacing="20" align="center">
        <tr>
            ...
            <td style="margin: 10px; padding: 10px;">
                <label>N&uacute;mero de Proceso en Coactivo </label>
            </td>
            <td style="margin: 10px; padding: 10px;">
...
<table id="traza" width="100%" style="f">
    <thead>
        <tr>
            ...
            <th style="display:none">N&uacute;mero del Titulo</th>
            ...
            <th>CIIU</th>
            <th>N&uacutemero de Resoluci&oacute;n</th>
            <th>Fecha de Resoluci&oacute;n</th>
            <th>Fecha Ejecutoria</th>
            <th>Fecha Avoca Conocimiento</th>
            <th>Documentos Juridico</th>
    ...
    <tbody>
        ...
        foreach ($datos->result_array as $col) {
            ...
            <tr>
                ...
                ?>
                <td style="display:none">Titulos Asignados al Proceso</td>
                <?php    
                }
                ?>
                <td><?= $col['NIT'] ?></td>
                ...
                <td style="display:none"><?= $col['COD_PROCESO_COACTIVO'] ?></td>
                <td><?= $col['CIIU'] ?></td>
                <td><?= $col['NUMERO_RESOLUCION'] ?></td>
                <td><?= $col['FECHA_RESOLUCION'] ?></td>
                <td><?= $col['FECHA_EJECUTORIA'] ?></td>
                <td><?= $col['FECHA_AVOCA'] ?></td>
                <td>
                    <form action= "<?php
                    // $fiscaliza = $col['COD_PROCESO_COACTIVO'];
                    $coactivo = $col['COD_PROCESO_COACTIVO'];
                    $nit = $col['NIT'];
                ...
                <td>
                    ...
                    <button class="btn btn-info at" id="traza" name="traza" at="<?= $col['COD_PROCESO_COACTIVO'] ?>" onclick="gestion('<?= $col['COD_PROCESO_COACTIVO'] ?>','<?= $col['TITULO'] ?>')"><i class="fa fa-eye"> Ver</i></button>
                    

                </td>

                <td align="center">
                    <form id="form" name="form" target = "_blank"  method="post" action="<?php echo base_url('index.php/consultarprocesos/pdfjuridico') ?>">
                        <br>
...
//    $(".at").click(function() {
        ...
        var url = "<?= base_url('index.php/consultarprocesos/verprocesojuridico') ?>";

        $('#cargar').load(url, {id: id,titulo:titulo});
