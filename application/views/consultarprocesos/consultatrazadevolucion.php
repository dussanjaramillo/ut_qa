...
<h1>Consulta Traza Devoluciones</h1>
<br>


<form action="<?php echo base_url('index.php/consultarprocesos/actualizatrazadevolucion'); ?>" name="form6" id="form6" method="POST">
    <table cellspacing="20" align="center">
        <tr>
            ...
            <td style="margin: 10px; padding: 10px;">
                <label>N&uacutemero Devoluci&oacute;n</label>
            </td>
            <td style="margin: 10px; padding: 10px;">
                <input name="DEVOLUCION" id="DEVOLUCION" onkeypress="return  soloNumeros(event)">
            </td>
...
<table id="traza" width="100%" style="table-layout: fixed;">
    <thead>
        <tr>
            <th>N&uacutemero Devoluci&oacute;n</th>
...
<!--            <th>Documentos Fiscalizaci&oacute;n</th>-->
    ...
    <tbody>
        ...
        foreach ($datos->result_array as $col) {
            ...
            <tr>
                <td style="word-wrap: break-word;"><?= $col['COD_DEVOLUCION'] ?></td>
                <td><?= $col['CODEMPRESA'] ?></td>
                <td><?= $col['RAZON_SOCIAL'] ?></td>
                <td><?= $col['TELEFONO_FIJO'] ?></td>
                <td><?= $col['DIRECCION'] ?></td>
                ...
                <td style="display:none"><?= $col['COD_REGIMEN'] ?></td>
                <td><?= $col['CIIU'] ?></td>
    <!--                <td>
                    <form action= "<?php
                $tal = $col['COD_DEVOLUCION'];
                ...
                <td>
                    <button class="btn btn-info at" id="traza" name="traza" at="<?= $col['COD_DEVOLUCION'] ?>" onclick="gestion('<?= $col['COD_DEVOLUCION'] ?>')"><i class="fa fa-eye"> Ver</i></button>

                </td>
                <td align="center">
                    <form id="form" name="form" target = "_blank"  method="post" action="<?php echo base_url('index.php/consultarprocesos/pdfdevolucion') ?>">
                        <br>
                        <button type="submit" name="pdf" id="pdf" class="btn btn-info"><i class="fa fa-file-pdf-o"> Pdf</i></button>
                        <input type="hidden" name="cod_devolucion" id="cod_devolucion" value="<?php echo $col['COD_DEVOLUCION']; ?>" >
                    </form>
                </td>
...
//    $(".at").click(function() {
        ...
        var url = "<?= base_url('index.php/consultarprocesos/verprocesodevolucion') ?>";
...
//    });
    ...
    function valida() {
        if ($("#CODEMPRESA").val() == "" && $("#NOMEMPRESA").val() == "" && $("#CIUDAD").val() == "" && $("#REGIONAL").val() == "" && $("#DEVOLUCION").val() == "") {
            document.forms.form6.action = "<?php echo base_url("index.php/consultarprocesos/consultatrazadevolucion") ?>";
        }
