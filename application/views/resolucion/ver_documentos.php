<table width="100%">
    <tr align="center" style="color:red">
        <td><b>No de Resoluci&oacute;n</b></td>
        <td><b><?php echo $documentacion_resolcuion[0]['NUMERO_RESOLUCION'] ?></b></td>
    </tr>
    <tr>
        <td>Resoluci&oacute;n Firmada</td>
        <td>
    ...
    <?php if (!empty($documentacion_recurso)) { ?>
        <tr>
            <td>Recursos Entregados</td>
            <td>
                <select name="resolucion" onchange="informacion(this, '2')">
                    <option value=""></option>
                    <?php
                    foreach ($documentacion_recurso as $documentacion) {
                        $informacion = Resolucion::documentacion_DOCUMENTOSRECURSO($documentacion['NUM_RECURSO']);
                        foreach ($informacion as $information) {
                            ?>
                            <option value="<?php echo $information['NOMBRE_DOCUMENTO'] ?>">Recurso del cliente -- <?php echo $information['NOMBRE_DOCUMENTO'] ?></option>
                            <?php
                        }
                    }
                    ?>
...
<script>
    ...
    function plantillas(dato, ruta) {
        ...
        var rutas = (ruta == 1) ? '<?php echo RUTA_DES ?>' + id + "/" + dato.value + ".txt" : '<?php echo RUTA_DES ?>' + id + "/" + id + "_Recurso/" + dato.value + ".txt";
    ...
    function pdf2(pdf) {
        $('#imprimir').hide();
        var html = '<a  href="<?php echo base_url() . RUTA_DES ?>' + id + "/" + pdf + '" target="_blank" style="color:red" align="center">Descargar Documento</a>';
