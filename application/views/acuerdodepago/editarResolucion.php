...
<script type="text/javascript">
    ...
    $('#guardar').click(function(){
        ...
        if (valida == true){
            ...
            $.post(url,{acuerdo:<?= $acuerdo ?>,estado:<?= $estado ?>,fiscalizacion:'<?= $fiscalizacion ?>',nit:<?= $nit ?>})
            .done(function(data){
                $('.conn').html(data);
                $(".ajax_load").hide("slow");
            }).fail(function (){
     ...
     $('#pdf').click(function() {
        tinyMCE.triggerSave();
        var url = "<?= base_url('index.php/acuerdodepago/pdf')?>";
        var url2 = "<?= base_url('index.php/acuerdodepago/guardarResolucion')?>";
