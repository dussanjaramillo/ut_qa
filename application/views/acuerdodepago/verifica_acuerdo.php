...
<table align='center'>
    ...
    <tr id='reciboPago' style="display:none">
            ...
            <td colspan='2' align='center'>
                <button id="<?= $cuotaDeuda ?>,<?= $date ?>,<?= $acuerdo ?>,<?= $fiscalizacion ?>,<?= $liquidacion ?>,<?= $nit ?>" type="button"  onclick="recibo(this.id)" class="generar btn btn-info" id="generar">RECIBO CUOTA CERO</button>
            <td>           
    </tr>
    <tr>
        <td id='mensaje_tipo'>
...
<script>
    $(document).ready(function() {        
        ...
        var mora = <?php echo @$mora ?>;
            ...
            switch (tipo){
                case 'condiciones':
                    msg = 'Acepta Condiciones y Garantías Propuestas?';
                break;
                ...
                break;
                case 'deudor_acuerdo':
                    msg = 'Deudor Firmó Acuerdo de Pago?';
                break;
                case 'acto_admin':
                    $('#check_decision').hide();
                    $('#guardar').hide();
                    msg = 'El Acuerdo fue notificado mediante Acto Administrativo';
                break;
                case 'acto_fin':
                    $('#check_decision').hide();
                    $('#guardar').hide();
                    msg = 'El Acuerdo ha Finalizado';
                break;
                ...
                case 'envio_coactivo':
                    $('#check_decision').hide();
                    $('#guardar').hide();
                    msg = 'Proceso Enviado a Coactivo';
                break;
            }
    ...
    $('#guardar').click(function() {       
        ...
        if (adicionales == undefined){
         ...
         var url = "<?= base_url('index.php/acuerdodepago/generarrecibo') ?>";
