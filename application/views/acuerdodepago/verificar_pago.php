...
$datacancel         = array('name'=>'button','id'=>'cancel-button','value' => 'Cancelar','type' => 'button','onclick' => 'window.location=\''.base_url().'index.php/acuerdodepago/gestionarAcuerdo\';','content' => '<i class="fa fa-minus-circle"></i> Cancelar','class' => 'btn btn-warning');
...
$fiscalizacion      = array('name'=>'fiscalizacion','id'=>'fiscalizacion','type'=>'hidden','value'=>$fiscalizacion);
...
<!--                        <th align='center'>RECIBO</th> -->
                 ...
                 <tr>  
                    ...
                    <td align="right">  
                        ...
                        if ($datos['PROYACUPAG_ESTADO'] == 1): 
                            ?>
                            <button disabled id="<?= $datos['PROYACUPAG_NUMCUOTA'] ?>,<?= $datos['PROYACUPAG_FECHALIMPAGO'] ?>,<?= $datos['PROYACUPAG_SALDO_INTACUERDO'] ?>,<?= $datos['PROYACUPAG_SALDO_INTCORRIENTE'] ?>,<?= $datos['PROYACUPAG_SALDO_CUOTA'] ?>,<?= $datos['NRO_LIQUIDACION']?>,<?= $datos['PROYACUPAG_CAPITALDEBE']?>" type="button"  onclick="recibo(this.id)" class="generar btn btn-success" id="generar">
                            RECIBO
                            </button>                                                                            
                            <?php
                            elseif ($datos['PROYACUPAG_ESTADO'] == 0 && $datos['DIAS_MORA'] > 30):
                                ...
                                ?>
                            <button id="<?= $datos['PROYACUPAG_NUMCUOTA'] ?>,<?= $datos['PROYACUPAG_FECHALIMPAGO'] ?>,<?= $datos['PROYACUPAG_SALDO_INTACUERDO'] ?>,<?= $datos['PROYACUPAG_SALDO_INTCORRIENTE'] ?>,<?= $datos['PROYACUPAG_SALDO_CUOTA'] ?>,<?= $datos['NRO_LIQUIDACION']?>,<?= $datos['PROYACUPAG_CAPITALDEBE']?>" type="button"  onclick="recibo(this.id)" class="generar btn btn-success" id="generar">
                            RECIBO
                            </button>                                                                            
                            <?php
                        endif;
...
<script>    
    ...
    function enviarMail(cuerpo, fecha, periodo){
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('index.php/acuerdodepago/enviaCorreo')?>",
    ...
    $('#guardar').click(function() {       
     ...
     function recibo(id){            
         ...
         var aporte        = <?= $aporte ?>;
         var url = "<?= base_url('index.php/acuerdodepago/generarTotalrecibo') ?>";
