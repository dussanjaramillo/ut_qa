...
<div style="background: #f0f0f0; width: 90%; margin: auto; overflow: hidden">
    ...
    <div class="notificacion_1" style="overflow: hidden; width: 90%; margin: 0 auto">
        ...
        <div  style="width: 80%; border-color: black; border: 1px solid grey; margin: auto; overflow: hidden;alignment-adjust: central ;padding: 15px 50px 0">
            <?php
            $ruta = base_url('uploads/soportes_remisibilidad/' . $cod_fiscalizacion . '/Expediente/' . $anexo["NOMBRE_DOCUMENTO"]);
            echo '<br>';
            echo form_label('<b>Archivo Adjunto </b><span class="required"></span>', 'lb_archivo');
            echo '<a href="' . base_url('uploads/soportes_remisibilidad/' . $cod_fiscalizacion . '/Expediente/' . $anexo["NOMBRE_DOCUMENTO"]) . '">' . $anexo["NOMBRE_DOCUMENTO"] . '</a>';
            echo '<br><br>';
    ...
    <script>
        ...
        $('#enviar').click(function() {
            ...
            $.post(url_notificacion, {cod_nulidad: cod_nulidad, cod_respuesta: estado, comentarios: comentarios})
                    .done(function(msg) {
                        if (estado == <?php echo NOTIFICACION_CORREO; ?>) {
