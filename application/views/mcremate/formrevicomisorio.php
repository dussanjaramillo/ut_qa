...
function returnList(){
    location.href = '<?=base_url()?>index.php/mcremate/listrevidespacomi';
}

function salvar(){
  
    
    var url = "<?php echo base_url()?>index.php/mcremate/updaterevicomisorio"; // El script a dónde se realizará la petición.
    $.ajax({
           type: "POST",
           url: url,
...
<div align="center" id="gestion" style="background: none repeat scroll 0 0 #FFFFFF;">
    ...
    <h3>Comentarios</h3>    
    <textarea id="comentarios" rows="10" style="width: 688px" disabled="disabled" name="comentarios"><?php echo $despachocomisorio->COMENTARIOS?></textarea>
        
    <hr>
    <h3>Comentarios de la aprobacion</h3>    
    <textarea id="comentarios" rows="10" style="width: 688px" name="observaciones_aprobacion"></textarea>
    <div class="controls controls-row"> 
        <div class="span4" align="left">
            <?php
            echo form_label('Documento Firmado', 'documento_sol_firmado');    

            $data = array(
                       'name'        => 'documento_sol_firmado',
                       'id'          => 'documento_sol_firmado',
                       'value'       => 'upload',
                       'class'       => 'span3',
                     );
        ...
        <div class="span4" align="left">
            <?php
            echo form_label('Estado', 'aprobado');                            
            $select = array();
            $select['']     = 'Seleccione';
            $select['S']    = 'APROBADO';
            $select['N']    = 'RECHAZADO';
            echo form_dropdown('aprobado', $select, '','id="aprobado" class="chosen span3" placeholder="seleccione..." ');
            ?>
        </div>
   ...
   <div class="span2">
        ...
        <div class="span2">
            ...
            $data = array(
                ...
                'type' => 'cancelar',
                'content' => 'cancelar',
                'onclick' => 'location.href=\'' . $url . 'index.php/mcremate/listrevidespacomi\''
            );
            echo form_button($data);
            ?>
        </div>
