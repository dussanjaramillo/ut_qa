...
<input type="hidden" id="cod_remisibilidad" name="cod_remisibilidad" value="<?php echo $cod_remisibilidad; ?>"readonly> 
<div class="preload"></div><img class="load" src="<?php echo base_url('img/27.gif'); ?>" width="128" height="128" />
<br>
<center>
    <br>
    <h1><?php echo 'Remisibilidad' ?></h1>
        ...
        <tbody>
            ...
            if (!empty($titulos)) {
                foreach ($titulos as $data) {
                    ?>             
                    <tr>          
                        <td><?php echo $data['COD_REMISIBILIDAD_DET']; ?></td>
                        <td><?= $data['CONCEPTO'] ?></td> 
                        <td><?= $data['NUM_LIQUIDACION'] ?></td> 
                        <td><?= $data['FECHA_LIQUIDACION'] ?></td> 
                        <td><?php echo '$ ' . number_format($data['VALOR_DEUDA'], 0, '.', '.'); ?></td> 
                        <td><input id="list_expediente" name="list_expediente[]" type="checkbox" onclick="recibir('<?= $data['HIJO'] ?>');" value="<?= $data['HIJO'] ?>"></td> 
                    </tr>
                    <?php
                }
            }
