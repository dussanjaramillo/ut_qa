...
<div id="textarea"  style="width: 90%; background: white; border-color: black; border: 1px solid grey; margin: auto; overflow: hidden;alignment-adjust: central ;padding: 15px 50px 0">
    ...
    <br><h2>Declaracion de Remisibilidad</h2>
    </div>';
    echo $Cabeza;
    ?>
    <center>
        <div id="codigo" class="codigo" style="alignment-adjust: central">
            <?php
            $data = array(
                'name' => 'cod_declaracion',
                'id' => 'cod_declaracion',
                'value' => 'CD' . $cod_remisibilidad,
                'class' => 'input-xlarge',
                'readonly' => 'readonly'
            );
            echo "<h2>Declaracion N° </h2>" . form_input($data);
            ?>
        </div>
    </center>
...
<script>
    ...
    $('#enviar').click(function() {
        ...
        var nombre = "Declaracion_Remisibilidad";
        var procede = "s";
        var comentarios = $('#comentarios').val();
        var consecutivo = $('#cod_declaracion').val();
        var fecha = "<?php echo $fecha_hoy; ?>";
        if (informacion == "" || nombre == "") {
            alert('No se puede continuar hasta que haya completado la declaracion');
            return false;
        }
        $(".preload, .load").show();
        var url = '<?php echo base_url('index.php/remisibilidad/guardar_archivo') ?>';
        var url_remisibilidad = '<?php echo base_url('index.php/remisibilidad/insertar_rem_declaracion') ?>';
        var cod_remisibilidad = "<?php echo $remisibilidad['COD_REMISIBILIDAD']; ?>";
        var cod_fiscalizacion = "<?php echo $remisibilidad['COD_FISCALIZACION']; ?>";
        var nombre_archivo = "<?php echo date("d-m-Y H:i:s"); ?>";
        nombre_archivo = nombre_archivo.replace(":", "_");
        nombre_archivo = nombre_archivo.replace(":", "_");
        nombre_archivo = nombre_archivo.replace(" ", "_");
        nombre_archivo = nombre_archivo + "_" + nombre;
        $.post(url, {nombre2: nombre, nombre: nombre_archivo, informacion: informacion, opcion: "1", cod_fiscalizacion: cod_fiscalizacion})
                .done(function(msg) {
                    alert('El archivo para Declaracion de Remisibilidad ha sido Guardado con Exito!!!');
                    $(".codigo").show();
        ...
        $.post(url_remisibilidad, {cod_remisibilidad: cod_remisibilidad, nombre_documento: nombre_archivo, comentarios: comentarios, procede: procede, fecha: fecha, consecutivo: consecutivo})
            ...
            alert('Error al Guardar la Declaracion');
            $(".preload, .load").hide();
        });
    });
    $('#generar').click(function() {
        ...
        var Consecutivo = '<br><div align="center"><br><h2>DECLARACION N° ' + $('#cod_declaracion').val() + '</h2><br></div>';
        var nombre = "Declaracion_Remisibilidad_" + $('#cod_declaracion').val();
        informacion = Cabeza + Consecutivo + informacion;
        if (informacion == "" || nombre == "") {
            alert('No se puede continuar hasta que haya realizado la declaracion de la Remisibilidad');
            return false;
        }
        $(".preload, .load").show();
