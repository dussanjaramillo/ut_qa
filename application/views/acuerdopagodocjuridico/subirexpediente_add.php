...
<div class="caja_negra" id="caja_negra" style="width: 60%; background: #F8F8F8; border-color: black; border: 1px solid grey; margin: auto; overflow: hidden;alignment-adjust: central ;padding: 15px 50px 0">
    ...
    echo form_open_multipart("acuerdopagojuridicodoc/Soporte_Expediente", $attributes);
    ?> 
    ...
    <center>
        <table>
            <tr>
                ...
                <td><input name="fecha_radicacion" value="<?= $date ?>" id="fecha_radicacion" type="text" class="span2" readonly required='true'/></td>                     
            </tr>
            <?php
            if ($cod_respuesta == 1284) {
                echo "<tr><td><b>Numero NIS*:</b></td>
                            <td><input type='text' name='numero_nis' id='numero_nis' class='span1' required='true'/></td></tr>";
            }
            ?>
        </table>                   
    </center>           
...
<script>
    ...
    $("#push").click(function() {
        ...
        var url5 = "<?php echo base_url("index.php/acuerdopagojuridicodoc/VistaPrevia_Documento"); ?>";
    ...
    function enviar() {
        if (cantidad_documentos == 0) {
            ...
            if (obligatorio && Descripcion != '' && Numero != '') {
