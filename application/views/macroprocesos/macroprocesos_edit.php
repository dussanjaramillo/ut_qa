...
<?php echo form_hidden('id',$result->CODMACROPROCESO) ?>
<h2>Editar macroproceso</h2>
<p>
<?php
 echo form_label('Nombre<span class="required">*</span>', 'nombre');
   $data = array(
              'name'        => 'nombre',
              'id'          => 'nombre',
              'value'       => $result->NOMBREMACROPROCESO,
              'maxlength'   => '128'
            );
 ...
 echo form_label('Ícono<span class="required">*</span>', 'icono');  
     ?> <select name="icono" id="icono" class="chosen" data-placeholder="seleccione..."> <?php
      foreach($iconos as $row) {
        if ($row->NOMBREICONO==$result->ICONO) {
...
<script>
    ...
    $("#borrar").click(function(evento){    
        ...
        $( "#dialog-confirm" ).dialog({
            ...
            buttons: {
                "Confirmar": function() {
                    location.href='<?php echo base_url()."index.php/macroprocesos/delete/".$result->CODMACROPROCESO; ?>';
...
<div id="dialog-confirm" title="¿Eliminar la macroproceso?" style="display:none;">
    <p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>¿Confirma que desea eliminar la macroproceso "<?php echo $result->NOMBREMACROPROCESO; ?>"?</p>
</div>