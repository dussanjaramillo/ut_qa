...
<table align="center">
    ...
    <tr>
        ...
        <td>Tipo concepto</td>
        <td><input type="text" id="concepto" value='<?= $concepto ?>' class="oblinit" readonly></td>
    </tr>
    ...
    <tr>
        <td>Pago Cuota Inicial</td>
        <td><input type="text" id="identificacion2" value='<?= '$ ' .number_format($vinicuota,0) ?>' class="oblinit" readonly></td>
        <td>Saldo de la Deuda</td>
        <td><input type="text" id="identificacion3" value='<?= '$ ' . number_format($capital-$vinicuota); ?>' class="oblinit" readonly></td>
    </tr>
    <tr>
...
<form id="form" method="post">
    ...
    <div align="right">
        ...
        <button type="button" id="atras" class="btn btn-warning" onclick="window.location = '<?= base_url('index.php/bandejaunificada/procesos') ?>'">Atras</button>   
    </div>
</form>
<div id="excel"></div>
...
<script>
    $(document).ready(function() {
        ...
        var url = "<?= base_url('index.php/acuerdodepagojuridico/cuotas') ?>";
        var juridico = '4';
        if (nit == '') {
    ...
    function recibo(id) {
        ...
        var url = "<?= base_url('index.php/acuerdodepagojuridico/generarrecibodepagocuotainicial') ?>";
    ...
    function datosActivo() {
        $("#preloadminicuota").show();
        $(".ajax_load").show("slow");
        var url = "<?= base_url('index.php/acuerdodepagojuridico/guardarAcuerdo') ?>";
        $('#dato_valida').val('0');
        $(".generar").show();
    ...
    function datosInactivo() {
        ...
        $.post(url, $('form').serialize({}))
                .done(function(msg) {
                    window.location = '<?= base_url('index.php/acuerdodepagojuridico/gestionarAcuerdo') ?>'
                })
                .fail(function(msg) {
                    $(".ajax_load").hide();
                });
...
//------------------------------------------------------------------------------ 
    ...
    $('#calcularidentificacion').click(function() {
        ...
        var cuotas = $('#cuotasiden').val();
        var url = "<?= base_url('index.php/acuerdodepagojuridico/tableacuerdopago') ?>";
        var alerta = "POR FAVOR INGRESAR LOS DATOS";
        var juridico = '4';

        if ((ejecutoria != "" || nit != "") && concepto != "" && cuotas != "") {
            $('#cuerpotabla').load(url, {cuotas: cuotas, juridico: juridico, ejecutoria: ejecutoria,
                nit: nit, concepto: concepto, cod_coactivo:<?= $cod_coactivo ?>, estado: '<?= $estado ?>'
            }, function(data) {
                $(".ajax_load").hide();
...
//------------------------------------------------------------------------------    
    ...
    $("#exportar").click(function() {
        $(".ajax_load").show("slow");
        var url = "<?= base_url('index.php/acuerdodepagojuridico/exportaExcel') ?>";
