...
for ($fila = 1; $fila <= $cantanio; $fila++) {
    ...
    for ($col = 1; $col <= 12; $col++) {
        ...
        echo "<td align='right'>
                ...
                <input type='text' style='width:80px' value='" . round($valores) . "'  readonly='readonly' id='cant" . $fila . $col . "' name ='cant" . $fila . $col . "[]'>
...
echo "</table>";
?>
<input type='hidden' style='width:80px' value='<?php echo $fila ?>'  id='filas' name ='filas'>
<input type='hidden' style='width:80px' value='<?php echo $col ?>'  id='columnas' name ='columnas'>
<input type='hidden' style='width:80px' value='<?php echo $cantidad ?>'  id='cantidadfilas' name ='$cantidadfilas'>            
            <input type="button" size="20" class="btn btn-success" value="Guardar" style="width:103px" id="guardar"  name="guardar" />
                        </form>
<script>
$(document).ready(function(){
    var numfilas = $("#$cantidadfilas").val();
    var totalfila = $("#filas").val()-1; 
    var columnas = $("#columnas").val()-1;   
    for (var fila = 1; fila <= totalfila; fila++){
        for (var col = 1; col <= columnas; col++) {
            var cant = $("#cant"+fila+col).val();
            var cantidad = $("#cantidad"+fila+col).val();
            if (cant != 0 && numfilas>1 ){
                var cantnew = parseInt(cant) + parseInt(cantidad);
                alert (cant+" + "+cantidad);
                alert (cantnew);
                $("#cantidad"+fila+col).val(cantnew);
                $("#cant"+fila+col).val(cantnew);
            } 
        }
    }
