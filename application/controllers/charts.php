...
class Charts extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->data['home'] = strtolower(__CLASS__).'/';
	...
	function index()
	{
		...
		$this->data['charts'] = $this->highcharts->set_serie($serie)->render();
		$this->template->load($this->template_file, 'charts/charts', $this->data);
	...
	{		
		...
		$this->data['charts'] = $this->highcharts->render(); // we render js and div in same time
		$this->template->load($this->template_file, 'charts/charts', $this->data);
	}
	...
	* Output data as array on json string
	...
	function data_get()
	{
		...
		$this->data['array']  = $this->highcharts->get_array();
		$this->data['json']   = $this->highcharts->get(); // false to keep data (dont clear current options)
		$this->template->load($this->template_file, 'charts/charts', $this->data);
	...
	function _data()
	{
		$this->data['users']['data'] = array(536564837, 444948013, 153309074, 99143700, 82548200);
		$this->data['users']['name'] = 'Users by Language';
		$this->data['popul']['data'] = array(1277528133, 1365524982, 420469703, 126804433, 250372925);
		$this->data['popul']['name'] = 'World Population';
		$this->data['axis']['categories'] = array('English', 'Chinese', 'Spanish', 'Japanese', 'Portuguese');
	...
	function _ar_data()
	{
		...
		foreach ($this->data['users']['data'] as $key => $val)
		{
			$output[] = (object)array(
				'users' 		=> $val,
				'population'	=> $this->data['popul']['data'][$key],
				'contries'		=> $this->data['axis']['categories'][$key]
			);
		}
		return $output;
	}
