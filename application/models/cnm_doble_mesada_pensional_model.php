<?php
class Cnm_doble_mesada_pensional_model extends CI_Model {
    ...
    return $datos;
        ...
        return $query->result_array[0]; 
					...
					function detalleCarteraDobleM ($id_cartera){
...
CNM_CARTERANOMISIONAL.COD_CARTERA_NOMISIONAL, CNM_CARTERANOMISIONAL.COD_TIPOCARTERA');
...
$this->db->select('CNM_EMPLEADO.IDENTIFICACION, CNM_EMPLEADO.NOMBRES, CNM_EMPLEADO.APELLIDOS, CNM_EMPLEADO.DIRECCION, CNM_EMPLEADO.TELEFONO,
CNM_EMPLEADO.TELEFONO_CELULAR, CNM_EMPLEADO.CORREO_ELECTRONICO,  REGIONAL.NOMBRE_REGIONAL, ESTADOCARTERA.DESC_EST_CARTERA,
CNM_CARTERANOMISIONAL.COD_CARTERA_NOMISIONAL, CNM_CARTERANOMISIONAL.COD_ESTADO, CNM_CARTERA_DOBLE_MESADA.ADJUNTOS, CNM_CARTERANOMISIONAL.COD_TIPOCARTERA');
...
$this->db->where("TIPOTASAINTERES.ELIMINADO IS NULL", NULL, FALSE); 
	...
	function detalleprevioDMesada($cod_cartera) {
    $this->db->select('CNM_CARTERA_DOBLE_MESADA.MOD_TASA_CORRIENTE, CNM_CARTERA_DOBLE_MESADA.MOD_TASA_MORA, CNM_CARTERANOMISIONAL.CALCULO_CORRIENTE, CNM_CARTERANOMISIONAL.CALCULO_MORA');
$this->db->join('CNM_CARTERANOMISIONAL', "CNM_CARTERANOMISIONAL.COD_CARTERA_NOMISIONAL=CNM_CARTERA_DOBLE_MESADA.COD_CARTERA AND CNM_CARTERA_DOBLE_MESADA.COD_CARTERA=".$cod_cartera);
$dato = $this->db->get("CNM_CARTERA_DOBLE_MESADA");

		
		 return $dato;
  }
	
			  function detalleDMesada($cod_cartera, $mod_t_corriente, $mod_t_mora, $calc_corr, $calc_mora) {
...
CNM_CARTERA_DOBLE_MESADA.ADJUNTOS, CNM_CARTERA_DOBLE_MESADA.NUMERO_RESOLUCION');
$this->db->select("to_char(CNM_CARTERA_DOBLE_MESADA.FECHA_INI_PENSION,'dd/mm/yyyy') AS FECHA_INI_PENSION",FALSE);
$this->db->select("to_char(CNM_CARTERA_DOBLE_MESADA.FECHA_RESOLUCION,'dd/mm/yyyy') AS FECHA_RESOLUCION",FALSE);
$this->db->select("to_char(CNM_CARTERA_DOBLE_MESADA.FECHA_ACTIVACION,'dd/mm/yyyy') AS FECHA_ACTIVACION",FALSE);
$this->db->join('CNM_CARTERANOMISIONAL', "CNM_CARTERANOMISIONAL.COD_CARTERA_NOMISIONAL=CNM_CARTERA_DOBLE_MESADA.COD_CARTERA AND CNM_CARTERA_DOBLE_MESADA.COD_CARTERA=".$cod_cartera);
$this->db->join('CNM_EMPLEADO', "CNM_CARTERANOMISIONAL.COD_EMPLEADO=CNM_EMPLEADO.IDENTIFICACION");
$this->db->join('REGIONAL', "CNM_EMPLEADO.COD_REGIONAL=REGIONAL.COD_REGIONAL");
$this->db->join('ESTADOCARTERA', "CNM_CARTERANOMISIONAL.COD_ESTADO=ESTADOCARTERA.COD_EST_CARTERA");
...
CNM_CARTERA_DOBLE_MESADA.NUMERO_RESOLUCION, CNM_CARTERANOMISIONAL.VALOR_DEUDA, CNM_CARTERANOMISIONAL.PLAZO_CUOTAS, 
CNM_CARTERANOMISIONAL.COD_PLAZO, CNM_CARTERANOMISIONAL.COD_FORMAPAGO, CNM_CARTERA_DOBLE_MESADA.MOD_TASA_CORRIENTE, CNM_CARTERA_DOBLE_MESADA.MOD_TASA_MORA,
