<?php
class Cnm_otras_carteras_model extends CI_Model {
	...
	return $row[$id];
        ...
        $query = $this->db->query(" SELECT ".$fields."  FROM ".$table." WHERE COD_ESTADO='1' AND TIPO='OTRAS'");
        return $query->result();
    }
...
CNM_CARTERANOMISIONAL.COD_CARTERA_NOMISIONAL, CNM_CARTERANOMISIONAL.COD_ESTADO, CNM_CARTERANOMISIONAL.COD_TIPOCARTERA, CNM_CARTERA_OTRAS_CARTERAS.ADJUNTOS, CNM_CARTERANOMISIONAL.COD_TIPOCARTERA');
$this->db->join('CNM_CARTERA_OTRAS_CARTERAS', "CNM_CARTERANOMISIONAL.COD_CARTERA_NOMISIONAL=CNM_CARTERA_OTRAS_CARTERAS.COD_CARTERA");
if(!empty($cedula)){
...
$this->db->where("TIPOTASAINTERES.ELIMINADO IS NULL", NULL, FALSE); 
		...
		return $dato;
			  ...
			  function detalleprevioOtrasC($cod_cartera) {
    $this->db->select('CNM_CARTERA_OTRAS_CARTERAS.COD_MODALIDAD, CNM_CARTERA_OTRAS_CARTERAS.MOD_TASA_CORRIENTE, CNM_CARTERA_OTRAS_CARTERAS.MOD_TASA_MORA, CNM_CARTERANOMISIONAL.CALCULO_CORRIENTE, CNM_CARTERANOMISIONAL.CALCULO_MORA');
$this->db->join('CNM_CARTERANOMISIONAL', "CNM_CARTERANOMISIONAL.COD_CARTERA_NOMISIONAL=CNM_CARTERA_OTRAS_CARTERAS.COD_CARTERA AND CNM_CARTERA_OTRAS_CARTERAS.COD_CARTERA=".$cod_cartera);
$dato = $this->db->get("CNM_CARTERA_OTRAS_CARTERAS");

		
		 return $dato;
  }
  
  		  function detalleOtrasC($cod_cartera, $cod_modalidad, $mod_t_corriente, $mod_t_mora, $calc_corr, $calc_mora) {

    $this->db->select('CNM_EMPLEADO.IDENTIFICACION, CNM_EMPLEADO.NOMBRES, CNM_EMPLEADO.APELLIDOS, CNM_EMPLEADO.DIRECCION, CNM_EMPLEADO.TELEFONO,
CNM_EMPLEADO.TELEFONO_CELULAR, CNM_EMPLEADO.CORREO_ELECTRONICO,  REGIONAL.NOMBRE_REGIONAL, ESTADOCARTERA.DESC_EST_CARTERA,
CNM_CARTERANOMISIONAL.COD_CARTERA_NOMISIONAL, TIPOCARTERA.NOMBRE_CARTERA, CNM_CARTERA_OTRAS_CARTERAS.NUMERO_RESOLUCION,
...
CNM_CARTERA_OTRAS_CARTERAS.ID_CODEUDOR, CNM_CARTERA_OTRAS_CARTERAS.NOMBRE_CODEUDOR, CNM_CARTERA_OTRAS_CARTERAS.ADJUNTOS, CNM_CARTERANOMISIONAL.COD_TIPOCARTERA');
$this->db->select("to_char(CNM_CARTERA_OTRAS_CARTERAS.FECHA_SOLICITUD,'dd/mm/yyyy') AS FECHA_SOLICITUD",FALSE);
$this->db->select("to_char(CNM_CARTERA_OTRAS_CARTERAS.FECHA_RESOLUCION,'dd/mm/yyyy') AS FECHA_RESOLUCION",FALSE);
$this->db->select("to_char(CNM_CARTERA_OTRAS_CARTERAS.FECHA_ACTIVACION,'dd/mm/yyyy') AS FECHA_ACTIVACION",FALSE);
$this->db->join('CNM_CARTERANOMISIONAL', "CNM_CARTERANOMISIONAL.COD_CARTERA_NOMISIONAL=CNM_CARTERA_OTRAS_CARTERAS.COD_CARTERA AND CNM_CARTERA_OTRAS_CARTERAS.COD_CARTERA=".$cod_cartera);
$this->db->join('CNM_EMPLEADO', "CNM_CARTERANOMISIONAL.COD_EMPLEADO=CNM_EMPLEADO.IDENTIFICACION");
$this->db->join('REGIONAL', "CNM_EMPLEADO.COD_REGIONAL=REGIONAL.COD_REGIONAL");
...
CNM_CARTERA_OTRAS_CARTERAS.COD_MODALIDAD, CNM_CARTERA_OTRAS_CARTERAS.NUMERO_RESOLUCION, CNM_CARTERANOMISIONAL.VALOR_DEUDA, CNM_CARTERANOMISIONAL.PLAZO_CUOTAS, 
CNM_CARTERANOMISIONAL.COD_PLAZO, CNM_CARTERANOMISIONAL.COD_FORMAPAGO, CNM_CARTERA_OTRAS_CARTERAS.MOD_TASA_CORRIENTE, CNM_CARTERA_OTRAS_CARTERAS.MOD_TASA_MORA,
...
CNM_CARTERA_OTRAS_CARTERAS.ID_CODEUDOR, CNM_CARTERA_OTRAS_CARTERAS.NOMBRE_CODEUDOR');
$this->db->select("to_char(CNM_CARTERA_OTRAS_CARTERAS.FECHA_SOLICITUD,'dd/mm/yyyy') AS FECHA_SOLICITUD",FALSE);
$this->db->select("to_char(CNM_CARTERA_OTRAS_CARTERAS.FECHA_RESOLUCION,'dd/mm/yyyy') AS FECHA_RESOLUCION",FALSE);
$this->db->select("to_char(CNM_CARTERA_OTRAS_CARTERAS.FECHA_ACTIVACION,'dd/mm/yyyy') AS FECHA_ACTIVACION",FALSE);
$this->db->join('CNM_CARTERA_OTRAS_CARTERAS', "CNM_CARTERANOMISIONAL.COD_CARTERA_NOMISIONAL=CNM_CARTERA_OTRAS_CARTERAS.COD_CARTERA");
